...
#wps.des: id = gap_filler2, title = Gap Filler Tool,
...
library(jsonlite)
#wps.off;
resource_id = "cuahsi-wdc-2016-11-18-63673712"
...
plot(DataValue~time, data=values1, type="l",col='red')
lines(DataValue~time, data=values1, type="l",col='green')
...
outputx = data.frame(date=time_conversion, row.names = NULL)
...
output_json = toJSON(list(xval = setNames(x, NULL),yval =setNames(y, NULL) ))
